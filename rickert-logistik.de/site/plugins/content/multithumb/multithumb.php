...
class plgContentMultithumb extends JPlugin {
	...
	function init_all($context) {
		...
		$this->botmtversion = 'Multithumb 3.4.0';
 ...
 white-space: pre-wrap;
   ...
   float: none;
	...
	function is_blog_by_context($context) {
		...
		$BLOG_CONTEXT=$this->_params->get('blog_context', "com_content.featured,com_content.category,mod_dn.featured" );
	...
	function is_article_by_context($context) {
		...
		$ARTICLE_CONTEXT=$this->_params->get('article_context', "com_content.article,mod_custom.content,mod_articles_category.content" );
	...
	function image_intro(&$images_json, $image_type, $params ) {
		...
		// echo "DEBUG:".gettype($images->image_fulltext)."<br>";
...
/*		if ( $this->_params->get('by_context' ) ) {
	...
	// Process one article
	function onPrepareContent ( $context, &$row, &$params, $page=0 ) {
...
$img = '<table '.$imgclass.' cellspacing="0" cellpadding="0" '.$style.' '.$align.' '.$hspace.' '.$vspace.' ><tr><td >'.$img.'</td></tr><tr><td class="'.$this->_params->get('caption_class').'" '.$caption_style.' >'.$this->caption.'</td></tr></table>';
						break;
					case "userdefined":
$img = '<div '.$imgclass.' '.$style.' '.$align.' '.$hspace.' '.$vspace.' >'.$img.'<span class="'.$this->_params->get('caption_class').'" '.$caption_style.' >caption text</span></div>';
						break;
				}
